...
var TableInit = function () {
	var oTableInit = new Object();
	    ...
	    oTableInit.Init = function () {
			$('#data_list_table').bootstrapTable({
		        url: basePath + '/action/collectorGroupDay/searchForm.do',     //请求后台的URL（*）
		        method: 'post',           //请求方式（*）
	...
	// 	        height: 460,            //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
		        ...
		        columns: [
	                  ...
	                  {field: 'sellPaymentCardCount',title: '当天售缴费卡张数', align:'center',valign:'middle',sortable:true,visible:false},
	                  {field: 'sellRefillCardCount',title: '当天售充值卡张数', align:'center',valign:'middle',sortable:true,visible:false},
	                  {field: 'sellPaymentCardFee',title: '当天售缴费卡金额', align:'center',valign:'middle',sortable:true,formatter : function(value){
					    	 return value / 100;
					     }},
	                 /* {field: 'sellRefillCardFee',title: '', align:'center',valign:'middle',sortable:true},*/
	                  {field: 'parkFee',title: '当天售充值卡金额', align:'center',valign:'middle',sortable:true,formatter : function(value){
					    	 return value / 100;
					     }}
	               ],
			});
	...
	oTableInit.queryParams = function (params) {
		...
		var temp = {  //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			endDate:$("#endDate").val(),
			collectorGroupId:$("#collectorGroupId").val(),
		};
		return temp;
	};
	return oTableInit;
	...
	var ButtonInit=function(){
		...
		oButtonInit.Init = function () {
			$('#searchBtn').click(function(){
				if(($('#collectorGroupId')).val()==null||($('#collectorGroupId')).val()==''){
...
function exports() {
	if ($('#data_list_table').bootstrapTable("getData").length > 0) {
		...
		window.open(basePath + '/collectorGroupDay/export.do?startDate='
				+ param.startDate + '&endDate=' + param.endDate
				+ '&collectorGroupId=' + param.collectorGroupId);
	} else {

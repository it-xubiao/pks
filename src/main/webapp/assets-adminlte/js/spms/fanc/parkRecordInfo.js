...
$('#operateTimeFrom').val( GetDateStr(0)+" 00:00:00" );
$('#operateTimeTo').val( GetDateStr(1)+" 00:00:00" );
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#data_list_table').bootstrapTable({
            url: basePath+'/action/parkRecord/dataList.do',     //请求后台的URL（*）
            method: 'post',           //请求方式（*）
...
// 	        height: 460,            //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            ...
            columns:[[
                ...
                {field:'berthCode',title:'泊位编号',align:'center',valign:'middle',sortable:true},
                {field:'plateNo',title:'车牌号码',align:'center',valign:'middle',sortable:true,formatter : function(value, row){
                	var tempPlateColor = ((row.plateColor==0) ? '蓝' : ((row.plateColor==1) ? '黄' : ((row.plateColor==2) ? '白' : ((row.plateColor==3) ? '黑' : null))))
                    return value == null ? '-' : (value+'[' + tempPlateColor + ']');
                }},
                ...
                {field:'fee',title:'实收金额',align:'center',valign:'middle',sortable:true,formatter : function(value){
                    return value / 100;
                }},
            ...
            onExpandRow : function(index, row, detail) {
                ...
                ddv.bootstrapTable({
                    url:basePath+'/action/parkRecord/queryOperateDetail.do?parkRecordId='+row.parkRecordId, //请求后台的URL（*）
                    method : 'get', //请求方式（*）
                    striped : true, //是否显示行间隔色
                    cache : false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
                    /*   showRefresh:true,
...
//                                }
                            	if (row.operateType == '05') {
                            		return "<img style=\"cursor: pointer\" alt=\"详情\" src=\""+basePath+"/assets-adminlte/images/look.png\" onclick=\"showPhoto('"+row.id+"')\"/>";
                              }
                            }
                        }],
	...
	oTableInit.queryOperateDetailParams = function (params) {
        var temp = {  //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            rows: params.limit,
            operateTimeTo : $('#operateTimeTo').val()
        };
        return temp;
    };
    oTableInit.queryParams = function(params) {
        var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            isDeparture : $('#isDeparture').val(),
            arriveTimeFrom : $('#arriveTimeFrom').val(),
            arriveTimeTo : $('#arriveTimeTo').val(),
            departureTimeFrom : $('#departureTimeFrom').val(),
            departureTimeTo : $('#departureTimeTo').val(),
            operateTimeFrom : $('#operateTimeFrom').val(),
            operateTimeTo : $('#operateTimeTo').val(),
        };
        return temp;
    };
...
var ButtonInit = function() {
    ...
    oButtonInit.Init = function() {
        $('#searchBtn').click(function() {
            ...
            $('#data_list_table').bootstrapTable('refresh',{url:basePath+ '/action/parkRecord/dataList.do',query:{page:1}});
        });
    }
    return oButtonInit;
};
...
function exports() {
	if($('#data_list_table').bootstrapTable("getData").length>0){
	    ...
	    window.open(basePath + '/action/parkRecord/export.do?parkId=' + param.parkId
	        + '&plateNo=' + param.plateNo 
	        + '&berthCode=' + param.berthCode
	        + '&regionId=' + param.regionId 
	        + '&streetId=' + $('#street_select0').val()
	        + '&plateColor=' + param.plateColor 
	        +'&feeState=' + param.feeState
	        + '&isManual=' + param.isManual
	        + '&isDeparture=' + param.isDeparture 
	        + '&arriveTimeFrom=' + param.arriveTimeFrom 
	        + '&arriveTimeTo=' + param.arriveTimeTo 
	        + '&departureTimeFrom=' + param.departureTimeFrom 
	        + '&departureTimeTo=' + param.departureTimeTo 
	        + '&operateTimeFrom=' + param.operateTimeFrom 
	        + '&operateTimeTo=' + param.operateTimeTo);
	}else{
		swal("提示", "当前查询条件下没有数据");
	}
}
...
function showPhoto(parkRecordId) {
	$.ajax({
		url : basePath+'/photoRecord/findPhotoById.do?operateDetailId='+parkRecordId,
		type : 'post',
		dataType : 'json',
		success : function(result){
			console.log(result.photoPath);
